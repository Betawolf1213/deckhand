...
class VoicePage(Page):
    ...
    def build(self) -> None:
        ...
        ttk.Button(bar, text="Clear", command=self.clear_history).pack(side="right")
        ttk.Button(bar, text="Reload commands", command=lambda: self.ctx.send({"type": "reload_config"})
                   ).pack(side="right", padx=6)
        ttk.Button(bar, text="Ping backend", command=lambda: self.ctx.send({"type": "ping"})).pack(side="right")
